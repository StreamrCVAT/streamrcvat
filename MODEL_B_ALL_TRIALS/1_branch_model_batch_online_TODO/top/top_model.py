...
def batch_train():
    ...
    top_model = construct_modelB_cnn(MODEL_HEIGHT, MODEL_WIDTH, 3, True)
    # top_model = construct_dnn(MODEL_HEIGHT, MODEL_WIDTH, 3)
    ...
    # hist = top_model.fit([X_train_image[:32,:,:,:], X_train_yolo[:32]], y_train[:32], epochs=15)
    hist = top_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=20)
    # hist = top_model.fit(X_train_yolo[:32], y_train[:32], epochs=60)
    # scores = top_model.evaluate([X_train_image, X_train_yolo], y_train)
    # print(scores)
    # y_pred = top_model.predict([X_train_image[:32,:,:,:], X_train_yolo[:32]])
    # y_pred = top_model.predict(X_train_image[:32,:,:,:])
    # y_pred = top_model.predict(X_train_yolo[:16])
    # print(y_pred)
    # y_pred =y_pred.T
    ...
    # hist = top_model.fit([X_train_image[16:,:,:,:], X_train_yolo[16:]], y_train[16:], epochs=15)
    # scores = top_model.evaluate([X_train_image, X_train_yolo], y_train)
    # print(scores)
    # y_pred = top_model.predict([X_train_image[32:,:,:,:], X_train_yolo[32:]])
    # y_pred = top_model.predict(X_train_image[32:,:,:,:])
...
def pipeline():
    ...
    top_model = batch_train()

    for layer in top_model.layers[:5]:
        print(layer)
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(batch_count == ONLINE_BATCH):
            ...
            X = np.array(batch_X)
            y = np.array(batch_y)
            # hist = top_model.fit(X, y, epochs=15, callbacks=[es])
            hist = top_model.fit(X, y, epochs=10)
            # hist = top_model.fit([[top_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
        ...
        if(line1 == line2): #no error
            ...
            if(line1[0] != line2[0]): #xmax do not match (top line eqn)
                yolo_human_errors.append(line1[0]-line2[0])
                top_part = cv2.resize(strips[3], (MODEL_HEIGHT, MODEL_WIDTH))
                # top_pre = top_model.predict([[top_part], [line1[3]]])
                ...
                if(((top_pre[0][0]+line1[0]) <= (line2[0]-3)) or ((top_pre[0][0]+line1[0]) >= (line2[0]+3))): #modelB wrongly predicts
                    print(filename, 'Model B predicts top WRONG')
                    # hist = top_model.fit([[top_part]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    # hist = top_model.fit([[top_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    batch_X.append(top_part) #append top cropped part ot batch_X for trianing
                    batch_y.append(line2[0]-line1[0]) #append top part error to batch y for traaining
                    batch_count += 1
                    # history['loss'].append(hist.history['loss'][0])
                    # history['mae'].append(hist.history['mae'][0])
                else:
                    ...
                    batch_X.append(top_part) #append top cropped part ot batch_X for trianing
                    batch_y.append(top_pre[0][0]) #append top part error to batch y for traaining
                    batch_count += 1
    ...
    if(save_i == 'y' or save_i == 'Y'):
        ...
        top_model.save_weights("model.h5")
