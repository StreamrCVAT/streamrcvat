...
UNFREEZ = 0 # 0 will always freeze top layers
...
def batch_train():
    ...
    left_model = construct_modelB_cnn(MODEL_HEIGHT, MODEL_WIDTH, 3, True)
    # left_model = construct_dnn(MODEL_HEIGHT, MODEL_WIDTH, 3)
    ...
    left_model.compile(optimizer='RMSprop', loss='mean_squared_error', metrics=['mae'])
    ...
    # hist = left_model.fit([X_train_image[:32,:,:,:], X_train_yolo[:32]], y_train[:32], epochs=15)
    hist = left_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=20)
    # hist = left_model.fit(X_train_yolo[:32], y_train[:32], epochs=60)
    # scores = left_model.evaluate([X_train_image, X_train_yolo], y_train)
    # print(scores)
    # y_pred = left_model.predict([X_train_image[:32,:,:,:], X_train_yolo[:32]])
    # y_pred = left_model.predict(X_train_image[:32,:,:,:])
    # y_pred = left_model.predict(X_train_yolo[:16])
    # print(y_pred)
    # y_pred =y_pred.T
    ...
    # hist = left_model.fit([X_train_image[16:,:,:,:], X_train_yolo[16:]], y_train[16:], epochs=15)
    # scores = left_model.evaluate([X_train_image, X_train_yolo], y_train)
    # print(scores)
    # y_pred = left_model.predict([X_train_image[32:,:,:,:], X_train_yolo[32:]])
    # y_pred = left_model.predict(X_train_image[32:,:,:,:])
...
def pipeline():
    ...
    left_model = batch_train()

    # for layer in left_model.layers[:5]:
    #     print(layer)
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(batch_count == ONLINE_BATCH):
            ...
            X = np.array(batch_X)
            y = np.array(batch_y)
            # hist = left_model.fit(X, y, epochs=15, callbacks=[es])
            hist = left_model.fit(X, y, epochs=10)
            # hist = left_model.fit([[left_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
        ...
        if(line1 == line2): #no error
            ...
            if(line1[1] != line2[1]): #xmax do not match (left line eqn)
                yolo_human_errors.append(line1[1]-line2[1])
                left_part = cv2.resize(strips[2], (MODEL_HEIGHT, MODEL_WIDTH))
                # left_pre = left_model.predict([[left_part], [line1[3]]])
                ...
                if(((left_pre[0][0]+line1[1]) <= (line2[1]-3)) or ((left_pre[0][0]+line1[1]) >= (line2[1]+3))): #modelB wrongly predicts
                    print(filename, 'Model B predicts left WRONG')
                    # hist = left_model.fit([[left_part]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    # hist = left_model.fit([[left_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    batch_X.append(left_part) #append left cropped part ot batch_X for trianing
                    batch_y.append(line2[1]-line1[1]) #append left part error to batch y for traaining
                    batch_count += 1
                    # history['loss'].append(hist.history['loss'][0])
                    # history['mae'].append(hist.history['mae'][0])
                else:
                    ...
                    batch_X.append(left_part) #append left cropped part ot batch_X for trianing
                    batch_y.append(left_pre[0][0]) #append left part error to batch y for traaining
                    batch_count += 1
    ...
    if(save_i == 'y' or save_i == 'Y'):
        ...
        left_model.save_weights("model.h5")
