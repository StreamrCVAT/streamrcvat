...
FRAME_HH = 720
FRAME_WW = 1280
MODEL_HEIGHT = FRAME_HH//10
MODEL_WIDTH =  FRAME_WW//6
...
def batch_train():
    ...
    #create the model
    # bottom_model = construct_modelB(MODEL_HEIGHT, MODEL_WIDTH, 3)
    ...
    hist = bottom_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=15)
...
def pipeline():
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(line1 == line2): #no error
            ...
            strips[2] /= 255.0
            strips[3] /= 255.0
            if(line1[2] != line2[2]): #ymax do not match (bottom line eqn)
                yolo_human_errors.append(line1[2]-line2[2])
                bottom_part = cv2.resize(strips[1], (MODEL_HEIGHT, MODEL_WIDTH))
                # bottom_pre = bottom_model.predict([[bottom_part], [line1[2]]])
                bottom_pre = bottom_model.predict([[bottom_part]])
                errors.append(bottom_pre[0][0])
                ...
                if(((bottom_pre[0][0]+line1[2]) <= (line2[2]-3)) or ((bottom_pre[0][0]+line1[2]) >= (line2[2]+3))): #modelB wrongly predicts
                    print(filename, 'Model B predicts bottom WRONG - FITTING Model B')
                    print('Error amount', bottom_pre[0][0])
                    # create image data augmentation generator
                    # datagen = ImageDataGenerator(width_shift_range=[-10, 10])
                    # prepare iterator
                    # samples = np.expand_dims(bottom_part, 0)
                    ...
                    # hist = bottom_model.fit(X_datagen, y_datagen, epochs=10, callbacks=[es])
                    hist = bottom_model.fit([[bottom_part]], [[line2[2]-line1[2]]], epochs=10, callbacks=[es])
                    # hist = bottom_model.fit([[bottom_part]], [[line2[1]-line1[1]]], epochs=2)
                    # hist = bottom_model.fit([[bottom_part], [line1[2]]], [[line2[2]-line1[2]]], epochs=10, callbacks=[es])
    ...
    # plot Model B outputs
    plt.plot(errors)
    plt.ylabel('Model B output')
    ...
    if(save_i == 'y' or save_i == 'Y'):
        ...
        with open("bottom_model.json", "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        bottom_model.save_weights("bottom_model.h5")
