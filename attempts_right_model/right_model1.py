...
def construct_dnn(height, width, depth):
    inp = Input(shape=(width, height, depth))
    flat = Flatten()(inp)
    hid1 = Dense(64, activation='relu')(flat)
...
def batch_train():
    BASE_SIZE = 32+32 #number of images to train for base model
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        image /= 255.0
        strips = strip(image, line1[1], line1[0], line1[3], line1[2], True) #args: img, xmin, ymin, xmax, ymax
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(line1 == line2): #no error
            ...
            if(line1[3] != line2[3]): #xmax do not match (right line eqn)
                ...
                # right_pre = right_model.predict([[right_part]])
                if(((right_pre[0][0]+line1[3]) <= (line2[3]-5)) or ((right_pre[0][0]+line1[3]) >= (line2[3]+5))): #modelB wrongly predicts
