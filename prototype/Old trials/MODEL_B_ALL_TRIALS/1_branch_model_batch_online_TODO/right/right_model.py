...
def batch_train():
    ...
    hist = right_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=20)
...
def pipeline():
    ...
    # for layer in right_model.layers[:5]:
    #     print(layer)
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(batch_count == ONLINE_BATCH):
            ...
            X = np.array(batch_X)
            y = np.array(batch_y)
            # hist = right_model.fit(X, y, epochs=15, callbacks=[es])
            hist = right_model.fit(X, y, epochs=10)
        ...
        if(line1 == line2): #no error
            ...
            if(line1[3] != line2[3]): #xmax do not match (right line eqn)
                ...
                if(((right_pre[0][0]+line1[3]) <= (line2[3]-3)) or ((right_pre[0][0]+line1[3]) >= (line2[3]+3))): #modelB wrongly predicts
                    print(filename, 'Model B predicts RIGHT WRONG')
                    # hist = right_model.fit([[right_part]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    # hist = right_model.fit([[right_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    batch_X.append(right_part) #append right cropped part ot batch_X for trianing
                    batch_y.append(line2[3]-line1[3]) #append right part error to batch y for traaining
                    batch_count += 1
                    # history['loss'].append(hist.history['loss'][0])
                    # history['mae'].append(hist.history['mae'][0])
                else:
                    ...
                    batch_X.append(right_part) #append right cropped part ot batch_X for trianing
                    batch_y.append(right_pre[0][0]) #append right part error to batch y for traaining
                    batch_count += 1
