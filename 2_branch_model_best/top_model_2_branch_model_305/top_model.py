...
def batch_train():
    ...
    #create the model
    top_model = construct_modelB(MODEL_HEIGHT, MODEL_WIDTH, 3)
    ...
    # hist = top_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=15)
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(line1 == line2): #no error
            ...
            if(line1[0] != line2[0]): #ymin do not match (top line eqn)
                ...
                top_part = cv2.resize(strips[3], (MODEL_HEIGHT, MODEL_WIDTH))
                top_pre = top_model.predict([[top_part], [line1[0]]])
                ...
                if(((top_pre[0][0]+line1[0]) <= (line2[0]-3)) or ((top_pre[0][0]+line1[0]) >= (line2[0]+3))): #modelB wrongly predicts
                    ...
                    # prepare iterator
                    # samples = np.expand_dims(top_part, 0)
                    ...
                    # hist = top_model.fit(X_datagen, y_datagen, epochs=10, callbacks=[es])
                    # hist = top_model.fit([[top_part]], [[line2[1]-line1[1]]], epochs=10, callbacks=[es])
                    hist = top_model.fit([[top_part], [line1[0]]], [[line2[0]-line1[0]]], epochs=10, callbacks=[es])
