...
def batch_train():
    ...
    #create the model
    left_model = construct_modelB(MODEL_HEIGHT, MODEL_WIDTH, 3)
    ...
    # hist = left_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=15)
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(line1 == line2): #no error
            ...
            if(line1[1] != line2[1]): #xmin do not match (left line eqn)
                ...
                left_part = cv2.resize(strips[2], (MODEL_HEIGHT, MODEL_WIDTH))
                left_pre = left_model.predict([[left_part], [line1[1]]])
                ...
                if(((left_pre[0][0]+line1[1]) <= (line2[1]-3)) or ((left_pre[0][0]+line1[1]) >= (line2[1]+3))): #modelB wrongly predicts
                    ...
                    # prepare iterator
                    # samples = np.expand_dims(left_part, 0)
                    ...
                    # hist = left_model.fit(X_datagen, y_datagen, epochs=10, callbacks=[es])
                    # hist = left_model.fit([[left_part]], [[line2[1]-line1[1]]], epochs=10, callbacks=[es])
                    hist = left_model.fit([[left_part], [line1[1]]], [[line2[1]-line1[1]]], epochs=10, callbacks=[es])
