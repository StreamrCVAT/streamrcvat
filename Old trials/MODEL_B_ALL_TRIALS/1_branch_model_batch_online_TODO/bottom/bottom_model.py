...
ONLINE_BATCH = 8 #number of frames to taake for batch train in online training
UNFREEZ = 5 # 0 will always freeze top layers
...
def batch_train():
    ...
    bottom_model = construct_modelB_cnn(MODEL_HEIGHT, MODEL_WIDTH, 3, True)
    # bottom_model = construct_dnn(MODEL_HEIGHT, MODEL_WIDTH, 3)
    ...
    bottom_model.compile(optimizer='RMSprop', loss='mean_squared_error', metrics=['mae'])
    ...
    # hist = bottom_model.fit([X_train_image[:32,:,:,:], X_train_yolo[:32]], y_train[:32], epochs=15)
    hist = bottom_model.fit(X_train_image[:32,:,:,:], y_train[:32], epochs=20)
    # hist = bottom_model.fit(X_train_yolo[:32], y_train[:32], epochs=60)
    # scores = bottom_model.evaluate([X_train_image, X_train_yolo], y_train)
    # print(scores)
    # y_pred = bottom_model.predict([X_train_image[:32,:,:,:], X_train_yolo[:32]])
    # y_pred = bottom_model.predict(X_train_image[:32,:,:,:])
    # y_pred = bottom_model.predict(X_train_yolo[:16])
    # print(y_pred)
    # y_pred =y_pred.T
    ...
    # hist = bottom_model.fit([X_train_image[16:,:,:,:], X_train_yolo[16:]], y_train[16:], epochs=15)
    # scores = bottom_model.evaluate([X_train_image, X_train_yolo], y_train)
    # print(scores)
    # y_pred = bottom_model.predict([X_train_image[32:,:,:,:], X_train_yolo[32:]])
    # y_pred = bottom_model.predict(X_train_image[32:,:,:,:])
...
def pipeline():
    ...
    bottom_model = batch_train()

    for layer in bottom_model.layers[:5]:
        print(layer)
    ...
    correct_model_a = 0
    correct_model_b_bottom = 0
    batch_count = 0 # for online learning
    batch_num = 0
    batch_X = list()
    batch_y = list()
    predictions = list()
    ...
    for filename in os.listdir(yolo_output_dir):
        ...
        if(batch_count == ONLINE_BATCH):
            batch_num += 1
            if(UNFREEZ>0 and batch_num%UNFREEZ == 0):
                for layer in bottom_model.layers[:5]:
                    layer.trainable = True
            print('Batch training the model')
            X = np.array(batch_X)
            y = np.array(batch_y)
            # hist = bottom_model.fit(X, y, epochs=15, callbacks=[es])
            hist = bottom_model.fit(X, y, epochs=10)
            # hist = bottom_model.fit([[bottom_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
        ...
        if(line1 == line2): #no error
            ...
            if(line1[2] != line2[2]): #xmax do not match (bottom line eqn)
                yolo_human_errors.append(line1[2]-line2[2])
                bottom_part = cv2.resize(strips[1], (MODEL_HEIGHT, MODEL_WIDTH))
                # bottom_pre = bottom_model.predict([[bottom_part], [line1[3]]])
                bottom_pre = bottom_model.predict([[bottom_part]])
                predictions.append(line2[2]-(line1[2]+bottom_pre[0][0]))
                # bottom_pre = bottom_model.predict([[bottom_part]])

                if(((bottom_pre[0][0]+line1[2]) <= (line2[2]-3)) or ((bottom_pre[0][0]+line1[2]) >= (line2[2]+3))): #modelB wrongly predicts
                    print(filename, 'Model B predicts bottom WRONG')
                    # hist = bottom_model.fit([[bottom_part]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    # hist = bottom_model.fit([[bottom_part], [line1[3]]], [[line2[3]-line1[3]]], epochs=10, callbacks=[es])
                    batch_X.append(bottom_part) #append bottom cropped part ot batch_X for trianing
                    batch_y.append(line2[2]-line1[2]) #append bottom part error to batch y for traaining
                    batch_count += 1
                    # history['loss'].append(hist.history['loss'][0])
                    # history['mae'].append(hist.history['mae'][0])
                else:
                    print(filename, 'Model B predicts bottom CORRECT')
                    batch_X.append(bottom_part) #append bottom cropped part ot batch_X for trianing
                    batch_y.append(bottom_pre[0][0]) #append bottom part error to batch y for traaining
                    batch_count += 1
                    # history['loss'].append(-1)
                    # history['mae'].append(-1)
                    correct_model_b_bottom += 1

    print('correct_model_a', correct_model_a)
    print('correct_model_b_bottom', correct_model_b_bottom)
    print(predictions)
    ...
    if(save_i == 'y' or save_i == 'Y'):
        ...
        model_json = bottom_model.to_json()
        with open("model.json", "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
        bottom_model.save_weights("model.h5")
